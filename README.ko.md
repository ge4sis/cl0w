...
[![Lines of Code](https://img.shields.io/badge/bot.py-~390줄-f59e0b?style=flat-square)]()
...
| | OpenClaw | cl0w |
|---|---|---|
| 핵심 로직 | 여러 파일, 2000줄 이상 | **단일 파일, ~390줄** |
| 의존성 | 15개 이상 | **4개 코어 + 선택** |
| 보안 | 사용자 허용 방식 | **ID allowlisting 및 Rate Limit** |
...
### 🔐 보안 우선 아키텍처
인바운드 포트를 열 필요가 없어 서버가 인터넷에 노출되지 않습니다. Telegram polling 방식을 통해 보안을 강화하며, Telegram ID allowlist와 Rate Limit으로 접근을 철저히 제어합니다. API 키는 환경변수로만 주입되며, 설정 파일에 평문으로 저장되지 않습니다.
...
### 🎭 Persona / Soul 시스템
에이전트에게 진짜 개성을 부여하세요. 마크다운 파일 하나를 작성하면 — 에이전트의 성격, 말투, 태도, 제약이 모든 LLM 요청의 system prompt에 자동으로 반영됩니다. cl0w는 기본적으로 `personas/default.md` 파일을 초기 Persona로 사용합니다.
...
`/persona technical` 명령으로 즉시 다른 Persona로 전환할 수 있습니다.
Persona 초기화: `/persona reset`
대화 세션 초기화: `/reset`
...
같은 코드를 비기술직 동료에게 설명할 때는 `/persona friendly` 로 전환하면 됩니다.
...
```
cl0w/
├── bot.py              ← 게이트웨이 전체 (~390줄)
├── config.yaml         ← 모든 설정 (프로바이더, MCP 서버, 한도)
├── personas/default.md ← 초기 활성 Persona (핫 리로드)
├── .env                ← 시크릿 (절대 커밋 금지)
...
| `/persona reset` | 기본 Persona로 리셋 (`personas/default.md`) |
| `/status` | 프로바이더 · 모델 · Persona · Tool 수 |
| `/reset` | 대화 히스토리 초기화 (현재 세션) |

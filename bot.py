...
async def _check_auth(update: Update) -> bool:
    ...
    if config.ALLOWED_USER_IDS and user.id not in config.ALLOWED_USER_IDS:
        ...
        await _reply_with_retry(update, "권한이 없습니다.")
        return False
    return True
...
async def _send_long(update: Update, text: str):
    ...
    for i in range(0, len(text), MAX):
        part = text[i:i + MAX]
        if part:
            await _reply_with_retry(update, part)
...
async def _typing(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        logger.warning(f"Ignored timeout/network error during typing action: {e}")
    except Exception as e:
        logger.warning(f"Ignored unexpected error during typing action: {e}")
...
async def new_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await _reply_with_retry(update, "대화 히스토리를 초기화했습니다. Persona는 유지됩니다.")
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await _reply_with_retry(update, text, parse_mode="Markdown")
...
async def persona_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not args:
        ...
        if p:
            ...
            await _reply_with_retry(update, f"현재 Persona: `{key}` (파일 없음)", parse_mode="Markdown")
        return
    ...
    if sub == "list":
        ...
        if not personas:
            await _reply_with_retry(update, "사용 가능한 Persona가 없습니다. `personas/` 폴더를 확인해주세요.")
            return
        ...
        if len(args) < 2:
            await _reply_with_retry(update, "사용법: `/persona set <name>`", parse_mode="Markdown")
            return
        target = args[1].lower()
        ...
        user_persona[user_id] = config.DEFAULT_PERSONA
        reset_session(user_id)
        await _reply_with_retry(update, "Persona를 기본값으로 초기화했습니다.")
...
async def skill_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not args or args[0].lower() == "list":
        ...
        if not skills:
            await _reply_with_retry(update, "사용 가능한 Skill이 없습니다. `skills/` 폴더를 확인해주세요.")
            return
...
async def mcp_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if sub == "list" or sub == "":
        if not manager:
            await _reply_with_retry(update, "MCP가 초기화되지 않았습니다.")
            return
        statuses = manager.get_status()
        if not statuses:
            await _reply_with_retry(update, "등록된 MCP 서버가 없습니다.\n`mcp.json` 파일을 확인해주세요.")
            return
        lines = []
        for s in statuses:
            icon = "✅" if s["running"] else "❌"
        ...
        await _reply_with_retry(update, "MCP 서버를 재시작하겠습니다...")
        if manager:
        ...
        await _reply_with_retry(update, f"MCP 재로드 완료. {running}/{len(statuses)} 서버 실행 중.")
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await _reply_with_retry(update, str(e))
        return
